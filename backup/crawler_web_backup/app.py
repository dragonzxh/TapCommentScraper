...
# 尝试导入爬虫模块
try:
    from steam_simple_crawler import SteamSimpleCrawler, JsonDataWriter, CsvDataWriter
except ImportError:
...
@app.route('/steam')
def steam_only():
    return render_template('steam_only.html')
...
def run_age_verification(game_id, use_headless=True):
    ...
    try:
        ...
        # 使用年龄验证模块处理
        result = age_verification.handle_age_verification_for_game(game_id, use_headless=use_headless)
...
def start_crawler():
    ...
    crawler_status = {
        ...
        "url": formatted_url,
        "headless": use_headless,  # 记录无头模式设置
        "max_reviews": max_reviews  # 记录最大评论数设置
    }
    
    # 调试信息 - 输出爬虫状态
    print(f"DEBUG - 爬虫状态设置: {crawler_status}")
    
    # 启动爬虫线程
    thread = threading.Thread(target=run_crawler, args=(crawler_type, formatted_url, use_headless, max_reviews))
    thread.daemon = True
    thread.start()
...
def run_crawler(crawler_type, url='', use_headless=True, max_reviews=None):
    ...
    # 调试信息 - 输出线程接收到的参数
    print(f"DEBUG - run_crawler线程接收到的参数: type={crawler_type}, url={url}, headless={use_headless}, max_reviews={max_reviews}")
    ...
    try:
        if crawler_type == "steam":
            ...
            if skip_login:
                # 使用简化版爬虫（不需要登录）
                crawler_status["log"].append("使用简化版Steam爬虫（无需登录）...")
                crawler_status["log"].append("服务器状态：加载简化版Steam爬虫依赖")
                crawler_status["progress"] = 10
                try:
                    # 修改这里，使用CsvDataWriter替代JsonDataWriter
                    ...
                    crawler = SteamSimpleCrawler(use_headless=use_headless, data_writer=data_writer)
                    
                    # 更新日志
                    crawler_status["log"].append("简化版Steam爬虫已启动")
...
def main():
    ...
    # 设置日志级别为WARNING以减少不必要的输出
    # 这会禁止INFO级别的日志（包括大多数访问日志）
    log.setLevel(logging.WARNING)
    
    # 禁用自动重载和调试模式，避免重复启动问题
...
def export_game_comments():
    ...
    # 查找CSV文件
    csv_file_pattern = f"*_评论_{app_id}.csv"
    ...
    export_filename = f"{safe_title}_评论_{app_id}.csv"
